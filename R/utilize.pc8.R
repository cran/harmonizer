...
utilize.pc8 <- function(b, e, firm.data, harmonized.data = NULL,
  ...
  if (is.null(harmonized.data)) {
    PC8_harm <- harmonize.pc8(b = b, e = e, progress = progress)
    mod_part <- 5
    ...
    if (!any(grep("PC8_", colnames(harmonized.data)))) {
    ...
    if (length(grep("PC8_", colnames(harmonized.data))) != (e - b + 1)) {
      stop("The entered harmonized data does not have enough columns which contain PC8 codes. It has ", length(grep("PC8_", colnames(harmonized.data))), " while ", (e - b + 1), " are needed for the entered time period. Please correct.")
    }
    if (suppressWarnings(!all(seq(from = b, to = e) == as.integer(substr(colnames(harmonized.data[grep("PC8_", colnames(harmonized.data))]), start = 5, stop = 8))))) {
      stop("The entered harmonized data does not match the given time period. Please correct.")
    }
    if (suppressWarnings(!all(years == as.integer(substr(colnames(harmonized.data[grep("PC8_", colnames(harmonized.data))]), start = 5, stop = 8))))) {
      stop("The entered harmonized data does not match the given firm data time period. Please correct.")
    }
    PC8_harm <- harmonized.data
    mod_part <- 1
  }
  ...
  syn_data <- PC8_harm[grep("f", PC8_harm$PC8plus), ]
  syn_codes <- syn_data[, grep("PC8_", colnames(syn_data))]
  ...
  yearly_nbr_firmssales <- rep(0, times = nbr_years - 1)
  yearly_add_prodsales <- rep(0, times = nbr_years - 1)
  yearly_drop_prodsales <- rep(0, times = nbr_years - 1)
  yearly_all_prodsales <- rep(0, times = nbr_years - 1)
